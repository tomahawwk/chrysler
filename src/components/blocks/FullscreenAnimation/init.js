...
export const FullscreenAnimation = () => {
  ...
  const toggleFullview = () => {
    ...
    if (transitionEffect.isFullscreen) {
      TweenLite.to(backToGridCtrl, 0.1, {
        ease: Quad.easeOut,
        opacity: 1,
        x: "100%"
      });
  ...
  backToGridCtrl.addEventListener("click", () => {
    document.querySelector('.modal--active').classList.add('modal--hide')
    document.querySelector('.modal--active').classList.remove('modal--active')
    backToGridCtrl.classList.remove('active')
    setTimeout(() => {
      document.querySelector('.modal--hide').classList.remove('modal--hide')
    }, 1000)
